...
from musculoco_il.algorithms.GAIL_KL_objective import Uniform2NormalKLDGAIL, TargetEntropyUniform2NormalKLDGAIL
...
def build_agent(mdp, expert_data, use_cuda, discrim_obs_mask, train_D_n_th_epoch=3,
                ...
                env_reward_frac=0.0, standardize_obs=True, action_divergence_coef=1e-3,
                target_entropy_mode=False, target_entropy=0.0):
    ...
    alg_params = dict(train_D_n_th_epoch=train_D_n_th_epoch,
                      ...
                      action_divergence_coef=action_divergence_coef,
                      a_dim=mdp_info.action_space.shape[0])

    print(f'USE_NEXT_STATES: {use_next_states}')
    print(f'ENV_REWARD_FRAC: {env_reward_frac}')

    if target_entropy_mode:
        alg_variant = TargetEntropyUniform2NormalKLDGAIL
        alg_params['target_entropy'] = target_entropy
    else:
        alg_variant = Uniform2NormalKLDGAIL

    agent = alg_variant(mdp_info=mdp_info, policy_class=OptionalGaussianTorchPolicy, policy_params=policy_params, sw=sw,
...
def experiment(n_epochs: int = 500,
               ...
               action_divergence_coef: float = 1e-3,
               target_entropy_mode: bool = False,
    ...
    if reward_type == 'target_velocity':
        ...
        if '4Ages' not in env_id:
            ...
            reward_type_loco = 'multi_target_velocity'
        if 'walk' in env_id:
    ...
    agent = build_agent(mdp=mdp, expert_data=expert_data, use_cuda=use_cuda,
                        ...
                        standardize_obs=standardize_obs, action_divergence_coef=action_divergence_coef,
                        target_entropy_mode=target_entropy_mode, target_entropy=target_entropy)
