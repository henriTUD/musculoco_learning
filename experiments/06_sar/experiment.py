...
from musculoco_il.algorithms.SAR import SAR_PCAICA, SAR_AutoEncoder
...
def build_agent(mdp, expert_data, use_cuda, discrim_obs_mask, train_D_n_th_epoch=3,
                ...
                env_reward_frac=0.0, standardize_obs=True):
    ...
    if isinstance(mdp, SARWrappedMuscleHumanoid):
        print("OBS DIM:")
        print(mdp.sar_module.dim_synergistic_space)
        ...
        policy_params = dict(network=FullyConnectedNetwork,
                             ...
                             output_shape=(mdp.sar_module.dim_synergistic_space,),
                             std_0=std_0,
    ...
    agent = GAIL_TRPO(mdp_info=mdp_info, policy_class=OptionalGaussianTorchPolicy, policy_params=policy_params, sw=sw,
...
def experiment(n_epochs: int = 500,
               ...
               n_sar_acquisition_epochs: int = 25,
               n_sar_fit_action_rollouts: int = 1000,
               sar_pol_std_0: float = 0.8,
               sar_pol_max_kl: float = 5e-3,
               sar_pol_entr_coef: float = 1e-3,
               sar_mode: str = 'pcaica',
               seed: int = 0):
    np.random.seed(seed)
    ...
    agent = build_agent(mdp=mdp, expert_data=expert_data, use_cuda=use_cuda,
                        ...
                        std_0=std_0, max_kl=max_kl, d_entr_coef=d_entr_coef, env_reward_frac=env_reward_frac,
    ...
    initial_log(core, tb_writer, logger_stoch, logger_deter, n_eval_episodes, gamma)

    if agent.policy._mu.model.network._stand is not None:
        agent.policy._mu.model.network._stand.unfreeze()

    # gail train loop
    for epoch in range(1, n_sar_acquisition_epochs):
        with catchtime() as t:
            start = time.time()
            core.agent.policy.deterministic = False
    ...
    print('--- Fitting SAR module...')
    sar_dataset = core.evaluate(n_episodes=n_sar_fit_action_rollouts)
    s, sar_fit_actions, *_ = parse_dataset(sar_dataset)

    if sar_mode == 'pcaica':
        sar_module = SAR_PCAICA(mdp.info.action_space.shape[0], 25)
    elif sar_mode == 'vae':
        encoder_params = dict(
            n_features=[64],
        ...
        sar_module = SAR_AutoEncoder(mdp.info.action_space.shape[0], 25,
                                     encoder_params, decoder_params)
    else:
        raise NotImplementedError

    sar_module.fit(sar_fit_actions)

    sar_save_path = os.path.join(results_dir, 'sar_module.msh')
    sar_module.save(sar_save_path)

    print('--- Building SAR env wrapper...')
    sar_env = SARWrappedMuscleHumanoid.generate(
        task=env_id.split(".")[1],
        gamma=gamma,
        horizon=horizon,
        n_substeps=n_substeps,
    ...
    sar_agent = build_agent(mdp=sar_env, expert_data=expert_data, use_cuda=use_cuda,
                            train_D_n_th_epoch=train_D_n_th_epoch, lrc=lrc,
                            lrD=lrD, sw=tb_writer, policy_entr_coef=sar_pol_entr_coef,
                            use_noisy_targets=use_noisy_targets, use_next_states=use_next_states,
                            last_policy_activation="identity", discrim_obs_mask=discrim_obs_mask,
                            std_0=sar_pol_std_0, max_kl=sar_pol_max_kl, d_entr_coef=d_entr_coef,
                            env_reward_frac=env_reward_frac, standardize_obs=standardize_obs)

    sar_core = Core(sar_agent, sar_env)

    for epoch in range(n_sar_acquisition_epochs, n_epochs):
        with catchtime() as t:
            sar_core.agent.policy.deterministic = False
            sar_core.learn(n_steps=n_steps_per_epoch, n_steps_per_fit=n_steps_per_fit, quiet=True, render=False)

            dataset = sar_core.evaluate(n_episodes=n_eval_episodes)
            J_mean = np.mean(compute_J(dataset))
            tb_writer.add_scalar("Eval_J", J_mean, epoch)
            agent_saver.save(sar_core.agent, J_mean)
            print('Epoch %d | Time %fs ' % (epoch + 1, float(t())))

            #### evaluate with deterministic policy
            sar_core.agent.policy.deterministic = True

            if sar_core.agent.policy._mu.model.network._stand is not None:
                sar_core.agent.policy._mu.model.network._stand.freeze()
            ...
            J_mean = np.mean(compute_J(dataset, gamma=gamma))
            L = np.mean(compute_episodes_length(dataset))
            E = sar_core.agent.policy.entropy()

            tb_writer.add_scalar("Eval_R-stochastic", R_mean, epoch)
            tb_writer.add_scalar("Eval_J-stochastic", J_mean, epoch)
            tb_writer.add_scalar("Eval_L-stochastic", L, epoch)

            a_abs_mean = log_action_mean(sar_core.agent, dataset)
            logger_stoch.log_numpy(Epoch=epoch, R_mean=R_mean, J_mean=J_mean, L=L, E=E,
                                   a_abs_mean=a_abs_mean)

            if sar_core.agent.policy._mu.model.network._stand is not None:
                sar_core.agent.policy._mu.model.network._stand.unfreeze()
